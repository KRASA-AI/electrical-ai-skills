...
- Load `config.yml` from the repo root for:
  ...
  - `inspection_report_generator.ahj_form_library[]` — the AHJ Inspection-Form Library (see below): per-AHJ report-number format, result vocabulary, required header/sign-off fields, citation format, and submission method, keyed by AHJ name
...
### AHJ Inspection-Form Library (config-driven, keyed by AHJ name)

Every AHJ has its own house style for an inspection report — the report-number scheme it expects, the header fields it requires, the citation format it reads, the result vocabulary it uses on the correction notice ("Approved / Partial / Disapproved" vs. "Pass / Pass-with-Corrections / Fail"), the re-inspection-fee trigger language, and the sign-off block (inspector license line, AHJ permit number, correction-deadline statement). A report formatted to the firm's generic house style gets re-typed by the office before it goes to the AHJ — which is exactly the manual step this skill exists to remove. The **AHJ Inspection-Form Library** pre-loads the matched AHJ's report template from config so the generated report already matches the form the AHJ expects, not a generic one.

**Config block** (`config.yml.inspection_report_generator`):

```yaml
inspection_report_generator:
  ahj_form_library:
    - ahj_name: "Multnomah County / Portland (OR)"
      report_number_format: "PDX-INS-YYYYMMDD-NNN"
      adopted_nec_cycle: "2023"            # cross-checks against the work order's AHJ cycle
      citation_format: "NEC 2023 §X.Y(Z); OAR 918-305 where amended"
      result_vocabulary: ["Approved", "Approved with Corrections", "Corrections Required", "Disapproved"]
      required_header_fields: ["permit_number", "inspection_type", "inspector_license", "parcel_id"]
      required_signoff: "Inspector license line + correction-deadline statement + re-inspection-fee note"
      reinspection_fee_trigger: "Second Disapproved on same scope triggers re-inspection fee"
      submission: "PDX Development Services portal; PDF upload keyed to permit #"
      verified_on: "2026-04-12"
    - ahj_name: "Clark County / Vancouver (WA)"
      report_number_format: "CLK-INS-YYYYMMDD-NNN"
      adopted_nec_cycle: "2023"
      citation_format: "NEC 2023 §X.Y(Z); WAC 296-46B where amended"
      result_vocabulary: ["Pass", "Pass with Corrections", "Fail"]
      required_header_fields: ["permit_number", "inspection_type", "inspector_license"]
      required_signoff: "WA L&I license line + EMERGENCY DISCONNECT labeling attestation on service work"
      reinspection_fee_trigger: "Per WA L&I fee schedule on second Fail"
      submission: "Clark County permitting portal"
      verified_on: "2026-04-12"
    - ahj_name: "King County / Seattle (WA)"
      report_number_format: "KC-INS-YYYYMMDD-NNN"
      adopted_nec_cycle: "2023"
      citation_format: "NEC 2023 §X.Y(Z); WAC 296-46B; Seattle Electrical Code where amended"
      result_vocabulary: ["Pass", "Corrections", "Fail"]
      required_header_fields: ["permit_number", "inspection_type", "inspector_license", "SEC_amendment_flag"]
      required_signoff: "WA L&I or Seattle DCI license line + labeling attestation"
      reinspection_fee_trigger: "Seattle DCI / WA L&I fee schedule"
      submission: "Seattle Services Portal or WA L&I, per jurisdiction split"
      verified_on: "2026-04-12"
  default_form: "generic"   # used when the property's AHJ is not in the library
```

**How the library is used:**

1. **AHJ match.** When the property's AHJ matches an `ahj_form_library` entry, render the report in that AHJ's template — its `report_number_format`, `required_header_fields`, `result_vocabulary` (use the AHJ's exact words in the Executive Summary's overall-assessment line), `citation_format`, and `required_signoff`. The library's `adopted_nec_cycle` is a **cross-check**, not an override — if it disagrees with the cycle on the work order, surface the conflict and use the work-order cycle (the NEC Cycle Check rule still governs which sections can be flagged 🟡).
2. **Inspection-Form Echo block.** At the very top of the generated report, above the header, surface a one-line **Form Echo**: `Form Echo — [AHJ]: template [report_number_format], result vocabulary [list], citation format [format], cycle cross-check [cycle] (verified [verified_on]).` This is the repo's standard Echo / Pre-load surfacing pattern — the office sees which AHJ template was applied before reading the report body, so a wrong-AHJ match is caught immediately.
3. **Result-vocabulary mapping.** Map the skill's internal severity roll-up to the AHJ's `result_vocabulary`: any open 🔴 → the AHJ's "Fail/Disapproved" word; 🟡 only → the AHJ's "Corrections" word; 🔵/✅ only → the AHJ's "Pass/Approved" word. Never substitute a generic word when the AHJ has a specific one on file.
4. **Freshness.** If a matched entry's `verified_on` is older than 90 days, append `[AHJ template last verified {date} — confirm portal/format hasn't changed]` to the Form Echo line.
5. **Never invent.** The library never fabricates an AHJ template, a report-number scheme, or a result vocabulary. If the property's AHJ is not in the library (or the config block is absent), the skill falls back to `default_form: generic` and behaves exactly as v1.1 did — the suggested `INS-YYYYMMDD-001` number, the generic Pass / Pass-with-Corrections / Fail / Recommendations-Only vocabulary, and the standard header. The library personalizes the form; it never changes the findings, the severity gating, or the NEC Cycle Check.

The Inspection-Form Library formats the report to the AHJ's expectations; it does not decide what passes. Severity, cycle gating, and the Panel-History flags below are unchanged by which template is applied.
...
### 1. Report Header
- **Form Echo line** (if an AHJ Inspection-Form Library template matched) — above the header, naming the applied AHJ template, result vocabulary, citation format, and cycle cross-check
- Company name, license number, contact info (from config)
- Property address and description
- Date of inspection
- Type of inspection
- Inspector name (from config or user input)
- Report number — use the matched AHJ's `report_number_format` from the library; fall back to `INS-YYYYMMDD-001` if no AHJ template matched
- Any AHJ-required header fields from the matched template (`required_header_fields` — e.g., permit number, parcel ID, SEC amendment flag)

### 2. Executive Summary
- 2-3 sentence overview of findings
- Overall assessment — use the matched AHJ's `result_vocabulary` exact words (e.g., Multnomah uses "Approved with Corrections"; Clark uses "Pass with Corrections"). Fall back to the generic Pass / Pass with Corrections / Fail / Recommendations Only when no AHJ template matched. Mapping: any open 🔴 → the AHJ's Fail/Disapproved word; 🟡 only → the AHJ's Corrections word; 🔵/✅ only → the AHJ's Pass/Approved word
...
### Example 4 — AHJ Inspection-Form Library: template match vs. generic fallback

The same set of findings produces a differently-headed report depending on whether the property's AHJ is in the firm's `ahj_form_library`. Below, a Portland final inspection (library **match**) and the Cleveland evaluation above (library **miss → generic fallback**), to show how the Form Echo and header adapt.

**(a) Library MATCH — Multnomah County / Portland (OR), final inspection, 1 open 🟡, 0 🔴:**

> **Form Echo — Multnomah County / Portland (OR): template PDX-INS-YYYYMMDD-NNN, result vocabulary [Approved / Approved with Corrections / Corrections Required / Disapproved], citation format "NEC 2023 §X.Y(Z); OAR 918-305 where amended", cycle cross-check 2023 (verified 2026-04-12).**
>
> **Report:** Electrical Final Inspection — [address], Portland OR
> **Permit #:** [from work order] · **Parcel ID:** [from work order] · **Inspector:** [name], License [OR-EC-…]
> **Report #:** PDX-INS-20260608-007
> **Overall assessment:** **Approved with Corrections** (Multnomah vocabulary; 1 open 🟡, 0 🔴)
> **Sign-off:** Inspector license line + correction-deadline statement + re-inspection-fee note (second Disapproved on same scope triggers re-inspection fee). Submit via PDX Development Services portal, PDF keyed to permit #.

**(b) Library MISS — City of Cleveland (OH), not in the firm's library → generic fallback (exactly v1.1 behavior):**

> *(No Form Echo line — AHJ not in `ahj_form_library`; `default_form: generic` applied.)*
> **Report #:** INS-20260427-016 (generic format)
> **Overall assessment:** Recommendations / Pass-with-Corrections (generic vocabulary)
> **Sign-off:** Standard inspector license line + standard disclaimer.

The findings, severity gating, and NEC Cycle Check are identical in both — only the report-number scheme, result vocabulary, required header fields, and sign-off block change with the matched template. A wrong-AHJ match is caught at a glance because the Form Echo names the applied template before the report body.
